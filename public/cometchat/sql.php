...
$sql_queries['admin_createGroup'] = "insert into cometchat_chatrooms (name,createdby,lastactivity,password,type) values ('{name}', '0','{lastactivity}','{password}','{type}')";
...
$sql_queries['admin_onlineusers'] = "select count(*) as users from (select DISTINCT cometchat.from userid from cometchat where cometchat.sent > {sent}-300 UNION SELECT DISTINCT cometchat_chatroommessages.userid userid FROM cometchat_chatroommessages WHERE cometchat_chatroommessages.sent > {sent}-300) x";
...
$sql_queries['getuserchatings'] = "select count(*) as users from (select DISTINCT cometchat.from userid from cometchat where cometchat.sent > {sent} UNION SELECT DISTINCT cometchat_chatroommessages.userid userid FROM cometchat_chatroommessages WHERE cometchat_chatroommessages.sent > {sent}) x";
$sql_queries['get_all_userchatings'] = "select count(*) as users from (select DISTINCT cometchat.from userid from cometchat UNION SELECT DISTINCT cometchat_chatroommessages.userid userid FROM cometchat_chatroommessages) x";
/*END: Report queries*/
...
$sql_queries['admin_importLanguage'] = "insert into `cometchat_languages` set `lang_key` = '{lang_key}', `lang_text` = '{lang_text}', `code` = '{code}', `type` = '{type}', `name` = '{name}' on duplicate key update `lang_key` = '{lang_key}', `lang_text` = '{lang_text}', `code` = '{code}', `type` = '{type}', `name` = '{name}';";
...
$sql_queries['admin_getbanneduserID'] = "(select {usertable_userid} id, {usertable_username} username from {usertable} where {usertable_username} LIKE '%{username}%') ";
...
$sql_queries['admin_viewuser_guestpart'] = " UNION select distinct(f.id) id, concat('{guestnamePrefix}',f.name) username from cometchat m1, cometchat_guests f where (f.id = m1.from and m1.to = '{userid}') or (f.id = m1.to and m1.from = '{userid}') UNION select distinct(f.id) id, concat('Guest-',f.name) username from cometchat_archive m2, cometchat_guests f where (f.id = m2.from and m2.to = '{userid}') or (f.id = m2.to and m2.from = '{userid}')";
$sql_queries['admin_viewuser'] = "select distinct(f.id) id, f.name username from cometchat m1, users f where (f.id = m1.from and m1.to = '{userid}') or (f.id = m1.to and m1.from = '{userid}') UNION select distinct(f.id) id, f.name username from cometchat_archive m2, users f where (f.id = m2.from and m2.to = '{userid}') or (f.id = m2.to and m2.from = '{userid}') {guestpart} order by username";
...
$sql_queries['admin_viewconversation'] = "select m.* from cometchat m where (m.from = '{userid}' and m.to = '{userid2}') or (m.to = '{userid}' and m.from = '{userid2}') UNION select m1.* from cometchat_archive m1 where (m1.from = '{userid}' and m1.to = '{userid2}') or (m1.to = '{userid}' and m1.from = '{userid2}') order by id desc";
...
$sql_queries['admin_viewuserchatroomconversation'] = "select cometchat_chatroommessages.*, f.{usertable_username} username  from cometchat_chatroommessages join {usertable} f on cometchat_chatroommessages.userid = f.{usertable_userid} where chatroomid = '{chatroomid}' UNION select cometchat_chatroommessages_archive.*, f1.{usertable_username} username  from cometchat_chatroommessages_archive join {usertable} f1 on cometchat_chatroommessages_archive.userid = f1.{usertable_userid} where chatroomid = '{chatroomid}' order by id desc LIMIT 200";
...
$sql_queries['admin_monitor_desc'] = " desc";
$sql_queries['admin_monitor_criteria2'] = " desc limit 20 ";
...
$sql_queries['admin_configeditor'] = "replace into `cometchat_settings` (`setting_key`,`value`, `key_type`) values {insertvalues}";
$sql_queries['admin_languageeditor'] = "insert into `cometchat_languages` set `lang_key` = '{lang_key}', `lang_text` = '{lang_text}', `code` = '{code}', `type` = '{type}', `name` = '{name}' on duplicate key update `lang_key` = '{lang_key}', `lang_text` = '{lang_text}', `code` = '{code}', `type` = '{type}', `name` = '{name}'";
$sql_queries['admin_coloreditor_insertvalues'] = "('{name}', '{value}', '{color_name}'),";
$sql_queries['admin_coloreditor'] = "replace into `cometchat_colors` (`color_key`,`color_value`, `color`) values {insertvalues}";
...
$sql_queries['getBlockedUserIDs_send'] = "select group_concat(blockedid) blockedids from ({querystring} select toid as blockedid from cometchat_block where fromid = '{userid}') as blocked";
$sql_queries['getBlockedUserIDs_receive'] = "select group_concat(toid) blockedids from cometchat_block where fromid = '{userid}'";
...
$sql_queries['getChatboxData'] = "select * from {tbl} as cometchat where ((cometchat.from = {from} and cometchat.to = {to} and direction <>1) or ( cometchat.from = {fromid} and cometchat.to = {toid} and direction <>2 )) and cometchat.direction <> 3 order by cometchat.id desc {prelimit};";
$sql_queries['getChatboxData_prependcondition'] = " and (cometchat.id < {id}) ";
$sql_queries['getChatboxData_prepend'] = "select * from {tbl} as cometchat where ((cometchat.from = {from} and cometchat.to = {to} and direction <>1) or ( cometchat.from = {fromid} and cometchat.to = {toid} and direction <> 2)) {prepend} and cometchat.direction <> 3 order by cometchat.id desc limit {prelimit};";
...
$sql_queries['getchatroombannedusers'] = "select GROUP_CONCAT(cometchat_chatrooms_users.userid) bannedusers from cometchat_chatrooms_users where isbanned=1 and chatroomid='{chatroomid}'";
...
$sql_queries['getBlockedUsers_guestpart'] = " UNION (select distinct(m.id) `id`, concat('{guestnamePrefix}',m.name) `name`, '' avatar from cometchat_block, cometchat_guests m where m.id = toid and fromid = '{fromid}')";
...
$sql_queries['chathistory'] = "	select cometchat.id, cometchat.message, cometchat.sent, cometchat.read, cometchat.sent, fromusertable.username as fromu ,tousertable.username as tou from cometchat
	...
	where
	cometchat.id in
	(	select min(id) from cometchat where (cometchat.from = '{userid}' AND cometchat.to = '{chatroomid}' AND cometchat.direction <> 1 ) OR (cometchat.to = '{userid}' AND cometchat.from = '{chatroomid}' AND cometchat.direction <> 2 ) AND cometchat.direction <> 3 group by Floor(cometchat.sent/86400)
	)";
...
// $sql_queries['checkUserExists'] = "select userid as count from ".TABLE_PREFIX.DB_USERTABLE." where {field} = '{value}'";
...
$sql_queries['deleteGroupUserApi'] = "delete from cometchat_chatrooms_users (chatroomid) values ('{chatroomid}') ";
$sql_queries['setBaseUrl'] = "replace into cometchat_settings(`setting_key`,`value`,`key_type`) values('BASE_URL','{baseurl}','0')";
...
$sql_queries['setReadReceiptsettings'] = "insert into cometchat_status (userid,readreceiptsetting) values ('{userid}','{message}') on duplicate key update readreceiptsetting = '{message}'";
$sql_queries['setStatus'] = "insert into cometchat_status (userid,status) values ('{userid}','{message}') on duplicate key update status = '{message}'";
$sql_queries['insertGuest'] = "insert into cometchat_guests (name) values ('{name}')";
$sql_queries['insertStatus'] = "insert into cometchat_status (userid,isdevice) values ('{userid}','1') on duplicate key update isdevice = '1'";
$sql_queries['insertIsTyping'] = "insert into cometchat_status (userid,typingto,typingtime) values ('{userid}','{typingto}','{typingtime}') on duplicate key update typingto = '{typingto}', typingtime = '{typingtime}'";
$sql_queries['insertCometStatus'] = "insert into cometchat_status (userid,status) values ('{userid}','{message}') on duplicate key update status = '{message}'";
$sql_queries['insertStatusMessage'] = "insert into cometchat_status (userid,message) values ('{userid}','{message}') on duplicate key update message = '{message}'";
$sql_queries['cometchatSessionOpen'] = "insert into cometchat_session(`session_id`,session_lastaccesstime) values('{session_id}',NOW()) on duplicate key update session_lastaccesstime = NOW()";
$sql_queries['cometchatSessionWrite'] = "insert into cometchat_session set session_id = '{session_id}', session_data = '{session_data}' on duplicate key update session_data = '{session_data}'";
...
$sql_queries['api_creategroupuser'] = "insert into cometchat_chatrooms_users (userid,chatroomid) values ('{userid}','{chatroomid}') on duplicate key update chatroomid = '{chatroomid}'";
...
$sql_queries['insertChatroom'] = "insert into cometchat_chatrooms (name,createdby,lastactivity,password,type) values ('{name}','{createdby}','{lastactivity}','{password}','{type}')";
$sql_queries['insertChatroomUser'] = "insert into cometchat_chatrooms_users (userid,chatroomid) values ('{userid}','{chatroomid}') on duplicate key update chatroomid = '{chatroomid}'";
...
$sql_queries['cron_groupusers'] = "delete from cometchat_chatrooms_users left join cometchat_status on cometchat_chatrooms_users.userid = cometchat_status.userid where lastactivity < ({lastactivity}-3600)";
...
$sql_queries['getRoomName'] = "select `name` from `cometchat_chatrooms` where `id` = {chatroomid} ";
$sql_queries['limitclause'] = "LIMIT {limit}";
$sql_queries['clearPrivateMessage'] = "delete from cometchat where (cometchat.read = 1 and cometchat.sent<{time}-10800) OR cometchat.sent<{time}-604800";
$sql_queries['clearGuestEntries'] = "delete from cometchat_guests where id in (select userid from cometchat_status where cometchat_status.lastactivity<{time}-10800);delete from cometchat_status where cometchat_status.lastactivity< {time} -10800 and cometchat_status.userid > 10000000;delete from cometchat_chatrooms_users where userid in (select userid from cometchat_status where cometchat_status.lastactivity<{time}-10800)";

/**
	Archive table queries start
	Below queries will take backup all cometchat tables and stored it into respective archive tables
*/
$sql_queries['archive_privatemessage'] = "INSERT INTO cometchat_archive SELECT * from cometchat where (cometchat.read = 1 and cometchat.sent<{time}-10800) OR cometchat.sent<{time}-604800";
$sql_queries['archive_guestentries'] = "INSERT INTO cometchat_guests_archive SELECT * from cometchat_guests where id in (select userid from cometchat_status where cometchat_status.lastactivity<{time}-10800)";
$sql_queries['archive_groupmessages'] = "INSERT INTO cometchat_chatroommessages_archive SELECT * from cometchat_chatroommessages where sent < ({sent}-10800)";
$sql_queries['archive_groups'] = "INSERT INTO cometchat_chatrooms_archive SELECT * from cometchat_chatrooms where createdby <> 0 and lastactivity < ({lastactivity}- {timeout})";
$sql_queries['archive_groupusers'] = "INSERT INTO cometchat_chatrooms_users_archive SELECT * from cometchat_chatrooms_users where lastactivity < ({lastactivity}-3600)";
...
class SqlQueries{
	function getQueries(){
		...
		$sqlqueries['loginuser'] = "select userid, username, link, avatar, uid, friends, grp, displayname from ".TABLE_PREFIX.DB_USERTABLE." where `username` = '{username}' and `password` = '{password}'";
		...
		$sqlqueries['export_cometchat_group_messages'] = "select FROM_UNIXTIME(m.sent) as Time, Grouptbl.name as GroupName, fromuser.username as FromName, m.message as Message from (select * from cometchat_chatroommessages UNION select * from cometchat_chatroommessages_archive) as m left join ( select ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." as userid, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." as username from ".TABLE_PREFIX.DB_USERTABLE." union select id as userid, name as username from cometchat_guests ) as fromuser on fromuser.userid=m.userid left join cometchat_chatrooms AS Grouptbl  ON m.chatroomid = Grouptbl.id where DATE(FROM_UNIXTIME(m.sent)) > (NOW() - INTERVAL 7 DAY)";
		return $sqlqueries;
	}
}
