						...
						if(chatroommode == 1){
							...
							jqcc.ajax({
								...
								success : function(data) {
									...
									if(windowMode == 0){
										jqcc[theme].generateOutgoingAvchatData(id,data,'audiocall');
									}
								},
								error : function(data) {
								}
			...
			processControlMessage : function(controlparameters) {
				...
				switch(controlparameters.method){
					case 'endcall':
						...
						closeCCPopup('audiochat');
						break;
					case 'rejectcall':
						processedmessage = jqcc.ccaudiochat.getLanguage('user_busy');
						jqcc[theme].removeAVchatContainer(controlparameters.params.fromid);
						break;
					case 'noanswer':
...
jqcc(document).ready(function(){
	/**
	 - Old code: Backword compartibility
	...
	*/
	...
	jqcc('.acceptAudioChat').off("click").live('click',function(e){
	...
	jqcc('.audiochat_cancelcall').off("click").live('click',function(e){
		e.stopImmediatePropagation();
		var to = jqcc(this).attr('to');
		var grp = jqcc(this).attr('grp');
		jqcc.ccaudiochat.cancel_call(to,grp);
	});

	jqcc('.audiochat_rejectcall').off("click").live('click',function(e){
		e.stopImmediatePropagation();
		var to = jqcc(this).attr('to');
		var grp = jqcc(this).attr('grp');
		jqcc.ccaudiochat.reject_call(to,grp);
	});
});
