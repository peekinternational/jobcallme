			...
			if((($group['createdby'] == 0 || ($group['createdby'] <> 0 && $time - $group['lastactivity'] < $chatroomTimeout)) || $group['createdby'] == $userid) && ($group['type'] <> 3)){
				...
				if($group['type'] == 2 && !in_array($userid, $userList) && $group['createdby'] != $userid){
					...
					if(USE_COMET == 1 && COMET_CHATROOMS == 1 && in_array($group['id'], $joinedgroups)){
						$chatroomList[$chromeReorderFix.$group['id']]['cometid'] = md5('chatroom_'.$group.$key);
					}
					if(in_array($group['id'], $joinedgroups)){
...
function inviteusers(){
	global $chatrooms_language, $close, $embedcss, $groupid, $groupname, $password, $encodedgroupname, $userstoinvite;
	$base_url = BASE_URL;
	$response = array();
	$invitedusers = array();
	$response['success'] = false;
...
function unban(){
	global $userid, $groupid, $groupname, $password, $cc_layout, $chatrooms_language, $language, $embed, $embedcss, $guestsMode, $basedata, $chromeReorderFix;
	$base_url = BASE_URL;
